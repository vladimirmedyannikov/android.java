...
public class Odnoklassniki {
     ...
     */
//    @Deprecated
//    public static final Odnoklassniki createInstanceForTokens(final Context context, final String appId, final String appSecret, final String appKey) {
//        return createInstanceForTokens(context, appId, appKey);
//    }

    public static final Odnoklassniki createInstance(final Context context, final String appId, final String appKey) {
        return createInstanceForTokens(context, appId, appKey, null);
    }
     ...
     */
    public static final Odnoklassniki createInstanceForTokens(final Context context, final String appId, final String appKey, final String appSecret) {
        if ((appId == null) || (appKey == null)) {
            throw new IllegalArgumentException(context.getString(R.string.no_application_data));
        }
        if (sOdnoklassniki == null) {
            sOdnoklassniki = new Odnoklassniki(context.getApplicationContext(), appId, appKey, appSecret);
        }
     ...
     * You must always call {@link Odnoklassniki#createInstanceForTokens(Context, String, String, String)} before calling this method, or {@link IllegalStateException} will be thrown
     */
    ...
    public static final Odnoklassniki getInstance() {
        if (sOdnoklassniki == null) {
            throw new IllegalStateException("No instance available. Odnoklassniki.createInstanceForTokens() needs to be called before Odnoklassniki.getInstance()");
        }
        return sOdnoklassniki;
    }
    ...
    private Odnoklassniki(final Context context, final String appId, final String appKey, final String appSecret) {
    ...
    protected final String mAppSecret;
    ...
    public final void requestAccessAndRefreshTokens(Context context, boolean oauthOnly, String... scopes) {
        Intent authIntent = new Intent(context, OkAuthActivity.class);
        authIntent.putExtra("method", 1);
        authIntent.putExtra("client_id", mAppId);
        authIntent.putExtra("application_key", mAppKey);
        authIntent.putExtra("client_secret", mAppSecret);
        authIntent.putExtra("oauth_only", oauthOnly);
        authIntent.putExtra("scopes", scopes);
    ...
    public void performPosting(String attachment, boolean userTextEnabled, OkListener postingListener) {
        performPosting(attachment, userTextEnabled, null, null, postingListener);
    }
     ...
     */
    public void performPosting(String attachment, boolean userTextEnabled, String accessToken, String sessionSecretKey, OkListener postingListener) {
        this.mOkListener = postingListener;
        if (!TextUtils.isEmpty(accessToken)) {
            this.mAccessToken = accessToken;
        }
        if (!TextUtils.isEmpty(sessionSecretKey)) {
            this.mSessionSecretKey = sessionSecretKey;
        }
